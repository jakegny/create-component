...
program
  ...
  .option('-c, --connected', 'connect component in index.js')
...
function scaffold(componentDir) {
  if (program.story) {
    fs.writeFileSync(`${componentDir}/story.js`, Mustache.render(STORY_JS, {componentName}));
  }

  if (program.connected) {
    fs.writeFileSync(`${componentDir}/index.js`, Mustache.render(CONNECTED_INDEX_JS, {componentName}));
  } else {
    fs.writeFileSync(`${componentDir}/index.js`, Mustache.render(INDEX_JS, {componentName}));
  }

  if (program.style) {
    fs.writeFileSync(`${componentDir}/style.css`, Mustache.render(STYLE_CSS, {componentName}));
  }

  fs.writeFileSync(`${componentDir}/component.js`, Mustache.render(COMPONENT_JS, {componentName}));
  fs.writeFileSync(`${componentDir}/props.js`, Mustache.render(PROPS_JS, {componentName}));

}

const INDEX_JS = fs.readFileSync('./templates/index_template.js', 'utf8');
const CONNECTED_INDEX_JS = fs.readFileSync('./templates/connected_index_template.js', 'utf8');
...
const COMPONENT_JS = fs.readFileSync('./templates/story_template.js', 'utf8');
